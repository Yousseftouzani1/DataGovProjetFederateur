...
class TaxonomyEngine:
    ...
    def __init__(self, domains_path: Optional[str] = None):
        self.domains_path = (
            Path(domains_path)
            if domains_path
            else Path(__file__).parent / "taxonomie" / "domains"
        )
        ...
        # Ligne à ajouter:
        self.moroccan_patterns = MOROCCAN_PATTERNS

        # ✅ AJOUTER CETTE LIGNE - Initialiser avec les patterns hardcodés par défaut
        self.moroccan_patterns = MOROCCAN_PATTERNS
        ...
        if mongodb_patterns and len(mongodb_patterns) >= 47:
            ...
            print("⚠️  MongoDB load failed or incomplete, using hardcoded fallback")
    ...
    def _load_from_files(self):
        ...
        for filepath in self.domains_path.glob("*.json"):
            try:
                ...
                domain_id = domain_tax.get("metadata", {}).get(
                    "domain_id", filepath.stem
                )
                domain_name = domain_tax.get("metadata", {}).get(
    ...
    def _compile_moroccan_patterns(
        self,
    ...
    def _get_context(
        self, text: str, start: int, end: int, size: int = 30
    ) -> str:
    ...
    def _check_context(
        self, text: str, start: int, end: int, keywords: List[str]
    ) -> bool:
    ...
    def analyze(
        ...
        for entity_type, patterns in self.compiled_patterns.items():
            for pattern, metadata in patterns:
                for match in pattern.finditer(text):
                    ...
                    if ctx_required and not self._check_context(
                        text, match.start(), match.end(), ctx_required
                    ):
                        continue

                    sensitivity = self.sensitivity_calc.calculate(
                            ...
                            detections.append(
                                {
                                    ...
                                    "start": match.start(),
                                    "end": match.end(),
                                    "sensitivity_level": subclass.get(
        ...
        if domains:
            detections = [
                ...
                for d in detections
                if any(
                    dom.lower() in d.get("domain", "").lower()
                    for dom in domains
                )
            ]
